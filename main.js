...
class ShowPopoutModifyPermissions {
	...
	static _makeCharacterVisibleToPlayers(actor) {
		if (actor) {
			this._updatePermission(actor, ENTITY_PERMISSIONS.LIMITED);
		}
	}

	static _makeJournalVisibleToPlayers(journal) {
		if (journal) {
			this._updatePermission(journal, ENTITY_PERMISSIONS.OBSERVER);
		}
	}

	static _updatePermission(object, permission) {
		if (object.data.permission.default === ENTITY_PERMISSIONS.NONE) {
			object.update(
				{
					permission: {
